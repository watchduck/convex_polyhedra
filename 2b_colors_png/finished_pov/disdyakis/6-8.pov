...
#declare FactorVariants = array[5]{
	1.666,  // 0  disdyakis dodeca
	4/ sqrt(3),  // 1  rhombic dodeca
	4 / sqrt(3),  // 2  hexa
	4 / sqrt(3),  // 3  octa
	3.0894  // 4  deltoidal icositetra
}
#declare Factor = FactorVariants[VariantIndex];

#declare AbsVariants = array[5]{  // sequence in the arrays: 0, yellow, blue, red

	array[4]{ 0, (sqrt(2))*Factor, (3/7 + 6*sqrt(2)/7)*Factor, (6/7 + 9*sqrt(2)/7)*Factor },  // 0  disdyakis dodeca
	array[4]{ 0, 1*Factor, 1*Factor, 2*Factor },  // 1  rhombic dodeca
	array[4]{ 0, 1*Factor, 1*Factor, 1*Factor },  // 2  hexa
	array[4]{ 0, (2/3)*Factor, 1*Factor, 2*Factor },  // 3  octa
	array[4]{ 0, (sqrt(2)/7 + 4/7)*Factor, Factor, sqrt(2)*Factor },  // 4  deltoidal icositetra

}
#declare Abs = AbsVariants[VariantIndex];
...
#declare EdgesAndVertices = union{
	...
	#if (VariantIndex = 0)  // disdyakis dodeca
		#include "6-8_disdyakis.inc"
	#end
	#if (VariantIndex = 1)  // rhombic dodeca
		#include "6-8_rhombic.inc"
	#end
	#if (VariantIndex = 2)  // hexa
		#include "6-8_hexa.inc"
	#end
	#if (VariantIndex = 3)  // octa
		#include "6-8_octa.inc"
	#end
	#if (VariantIndex = 4)  // deltoidal icositetra
		#include "6-8_deltoidal.inc"
	#end
}
