...
#declare Abs = array[10]{ 
	0,
	(3*sqrt(5)/44 + 45/44)*Factor,
	(-sqrt(5)/2 + 5/2)*Factor,
	(15/22 + 6*sqrt(5)/11)*Factor,
	(3*sqrt(5)/10 + 3/2)*Factor,
	(sqrt(5))*Factor,
	(27*sqrt(5)/44 + 75/44)*Factor,
	(3/2 + 9*sqrt(5)/10)*Factor,
	(sqrt(5)/2 + 5/2)*Factor,
...
// declare factors used in 'ModifiedP' based on VariantIndex
	...
	#declare RedFactor = 1;
	#declare YellowFactor = 1;
#end
#if (VariantIndex = 1)  // rhombic triaconta
	#declare BlueFactor = sqrt(5)/2 - 1/6;
	#declare RedFactor = (5 * sqrt(5) - 5) / 6;
	#declare YellowFactor = 1;
#end
#if (VariantIndex = 2)  // dodeca
	#declare BlueFactor = sqrt(5)/2 - 1/6;
	#declare RedFactor = sqrt(5)/3;
	#declare YellowFactor = 1;
#end
#if (VariantIndex = 3)  // icosa
	#declare BlueFactor = sqrt(5)/2 - 1/6;
	#declare RedFactor = (5 * sqrt(5) - 5) / 6;
...
#declare EdgesAndVertices = union{
	...
	#if (VariantIndex = 0)  // disdyakis triaconta
		#include "12-20_disdyakis.inc"
	#end
	#if (VariantIndex = 1)  // rhombic triaconta
		#include "12-20_rhombic.inc"
	#end
	#if (VariantIndex = 2)  // dodeca
		#include "12-20_dodeca.inc"
	#end
	#if (VariantIndex = 3)  // icosa
		#include "12-20_icosa.inc"
	#end
	#if (VariantIndex = 4)  // deltoidal hexeconta
